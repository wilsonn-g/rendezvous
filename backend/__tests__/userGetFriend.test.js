...
describe("test GET /:userid/friends", () => {
  it ("should return a list of friends the user has", async (done) => {
        ...
        let getres = await request.get("/users/"+mockUser._id+"/friends").expect(200);
        expect(mockUser.friends.length == getres.body.length).toBe(true);
        let i = true;
        let j;
        for (j = 0; j < mockUser.friends.length; j++) {
          if (mockUser.friends[j]!=getres.body[j]) {
            i = false;
          }
        }
        expect(i).toBe(true);
  ...
  it ("should return a 404 error because it is looking for a userid that doesn't exist", async (done) => {
    ...
    let getres = await request.get("/users/"+badID+"/friends").expect(404);
    done();
  });
