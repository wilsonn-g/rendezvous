...
describe("test GET /:userid/groups", () => {
  it ("should return a list of groups the user is in", async (done) => {
        ...
        let getres = await request.get("/users/"+mockUser._id+"/bookmarked").expect(200);
        expect(mockUser.bookmarkedPlaces.length == getres.body.length).toBe(true);
        let i = true;
        let j;
  ...
  it ("should return a 404 error because it is looking for a userid that doesn't exist", async (done) => {
    ...
    let getres = await request.get("/users/"+badID+"/bookmarked").expect(404);
    done();
  });
